...
average_percentage_of_cells_within_radius <- function(spe_object, 
  ...
  reference_celltypes <- data[data[,feature_colname] 
                                        == reference_celltype,]
  target_celltypes <- data[data[,feature_colname] 
                                     == target_celltype,]
  ...
  if (nrow(reference_celltypes) == 0 || nrow(target_celltypes) == 0) {
    ...
    #frNN output ids
    search_result <- dbscan::frNN(data[,c("Cell.X.Position", 
                                                    "Cell.Y.Position")],
    ...
    if (length(rownums) == 0) {
      ...
      for(cell in names(search_result$id)){
        ...
        # Remove reference cell of course
        ...
        cell_IDs <- c(cell_IDs, data[cell,"Cell.ID"])
      }
