...
### Function for to compare 3D vs 2D ------------------------------------------
plot_3D_vs_2D_metric <- function(spes_table, 
                                 metric, 
                                 metric_df3D,
                                 metric_df2D,
                                 arrangement, 
  ...
  create_plot <- function(data, x_aes, y_aes, color_aes, title = "") {
    ...
    if (color_aes == "E_volume") {
      ...
      breaks <- pretty(c(min(data[[color_aes]]), max(data[[color_aes]])), n = 2)
      plot <- plot + 
        geom_point() + 
  ...
  # Add legends
  combined_plots_with_legends <- plot_grid(combined_plots, legends,
                                           nrow = 2, ncol = 1,
                                           rel_heights = c(1, 0.1))
