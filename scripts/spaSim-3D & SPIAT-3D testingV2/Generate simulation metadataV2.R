...
## Mixed clusters parameter
cluster_prop_B_range <- c("min" = 0.1, "max" = 0.5)   # Proportion of B cells in cluster will range from 0.5 to 0.9, proportion of A cells in cluster will be 1 - prop(B)
...
spes_table_colnames <- c("bg_prop_A", "bg_prop_B", "E_radius_x", "E_radius_y", "E_radius_z", "N_width", "cluster_prop_B", "ring_width_factor", "cluster_x_coord")
...
spes_table$cluster_prop_B <- cluster_prop_B_fixed
...
parameter_continuous_data <- list(
  ...
  cluster_prop_B = runif(n_simulations, cluster_prop_B_range["min"], cluster_prop_B_range["max"]),
...
setwd("~/R/spaSim-3D/scripts/spaSim-3D & SPIAT-3D testingV2")
  ...
  for (shape in shapes) {
    ...
    for (i in seq(nrow(spes_table_subset))) {
      ...
      if (arrangement == "mixed") {
        ...
        curr_metadata$cluster_1$cluster_cell_proportions <- c(1 - spes_table_subset$cluster_prop_B[i], spes_table_subset$cluster_prop_B[i])
        ...
        curr_metadata$cluster_1$centre_loc <- c(spes_table_subset$cluster_x_coord[i], 300, 300)
