...
check_input_parameters <- function(input_parameters) {
  ...
  if (check_value == 0) {
    ...
    for (i in seq(nrow(pairs))) {
      name1 <- pairs[["name1"]][i]
      name2 <- pairs[["name2"]][i]
      if (name1 %in% input_parameter_names && length(input_parameters[[name1]]) != length(input_parameters[[name2]])) {
        check_value <- 14
...
### Example code to run -------------------------------------------------------
# simulated_cluster_spe <- spaSim3D_cluster_integrator(simulated_background_spe)
