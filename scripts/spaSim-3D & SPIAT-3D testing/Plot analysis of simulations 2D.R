...
# Duplicate spe_table by 5
for (i in seq(length(arrangement_tables))) {
  arrangement_tables[[i]] <- arrangement_tables[[i]] %>%
    mutate(row_num = row_number())
  arrangement_tables[[i]] <- do.call(bind_rows, replicate(5, arrangement_tables[[i]], simplify = FALSE)) %>%
    arrange(row_num)
  arrangement_tables[[i]]$row_num <- NULL
}

### Set up plots metadata --------------------------------------------------------
non_gradient_plots_metadata <- list(
  slice = list(x_aes = "slice", y_aes = "metric"),
...
gradient_plots_metadata <- list(
  slice = list(x_aes = "gradient", y_aes = "metric", color_aes = "slice"),
...
for (arrangement in arrangements) {
  setwd(paste("~/R/spaSim-3D/scripts/spaSim-3D & SPIAT-3D testing/analysis2D_tables/", arrangement,  sep = ""))
  for (metric in metrics) {
    if (metric == "prop_AUC") {
      ...
      metric_dfs[[arrangement]][[metric]] <- read.table(paste(arrangement, "slices", metric, "df.csv", sep = "_")) 
    }
  }
}
...
### Middle slice only (slice == 3) and no bg noise -----------------------------
# Read tables
setwd("~/R/spaSim-3D/scripts/spaSim-3D & SPIAT-3D testing/spe_tables")
mixed_spes_table <- read.table("mixed_spes_table.csv")
ringed_spes_table <- read.table("ringed_spes_table.csv")
...
for (arrangement in arrangements) {
  ...
  for (metric in metrics) {
    if (metric == "prop_AUC") {
      ...
      df <- read.table(paste(arrangement, "slices", metric, "df.csv", sep = "_")) 
      df <- df[df$slice == 3, ]
...
setwd("~/R/plots/2D_middle_slice_no_bg_noise")
saveRDS(metric_plots, "metric_plots_2D_middle_slice_no_bg_noise.RDS")
...
metrics <- c("AMD", "MS", "NMS", "ACINP", "AE", "ACIN", "CKR", "prop_SAC", "prop_prevalence", "prop_AUC", "entropy_SAC", "entropy_prevalence", "entropy_AUC")
arrangements <- c("mixed", "ringed", "separated")
metric_plots <- readRDS("metric_plots_2D_middle_slice_no_bg_noise.RDS")
