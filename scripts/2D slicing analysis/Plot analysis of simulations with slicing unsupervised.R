...
plot_AMD_metric <- function(spes_table, AMD_df, slices_AMD_df, arrangement_colname) {
  ...
  for (i in seq(nrow(AMD_pairs))) {
    ...
    slices_AMD_df_subset$AMD <- joint_df$AMD_2D - joint_df$AMD_3D
    ...
    fig_slice <- ggplot(plot_df, aes(!!sym(arrangement_colname), AMD, col = slice)) +
      geom_point() +
      theme_bw() +
      scale_color_manual(values = c("lightblue", "yellow", "red", "orange", "blue"))
...
plot_gradient_metrics_type1 <- function(spes_table, gradient_metric_df, slices_gradient_metric_df, metric, arrangement_colname) {
  ...
  for (i in seq(length(cell_types))) {
    ...
    for (radii_colname in radii_colnames) {
      slices_gradient_metric_df_subset[ , radii_colname] <- joint_df[ , paste(radii_colname, "_2D", sep = "")] - joint_df[ , paste(radii_colname, "_3D", sep = "")]
    }
    ...
    all_plots_list[[cell_types[i]]] <- list(arrangement = fig_arrangement + theme(legend.position = "none"), 
                                            slice = fig_slice + theme(legend.position = "none"))
                                            # bg_prop_A = fig_bg_prop_A + theme(legend.position = "none"), 
                                            # bg_prop_B = fig_bg_prop_B + theme(legend.position = "none"),
                                            # shape = fig_shape + theme(legend.position = "none"),
                                            # # variation_E = fig_variation_E + theme(legend.position = "none"),
  ...
  for (i in seq(length(cell_types))) {
    ...
    plots <- plot_grid(all_plots_list[[reference_cell_type]]$arrangement,
                       all_plots_list[[reference_cell_type]]$slice,
...
plot_gradient_metrics_type1_boxplot <- function(spes_table, gradient_metric_df, slices_gradient_metric_df, metric, arrangement_colname) {
  ...
  for (i in seq(length(cell_types))) {
    ...
    fig_boxplot <- ggplot(plot_df, aes(variable, value)) +
      geom_boxplot() +
      labs(x = "radius", y = metric) +
      theme_bw()
    
    all_plots_list[[cell_types[i]]] <- list(fig_boxplot = fig_boxplot + theme(legend.position = "none"))
  }
...
plot_gradient_metrics_type2 <- function(spes_table, gradient_metric_df, slices_gradient_metric_df, metric, arrangement_colname, min_radius, max_radius) {
  ...
  for (i in seq(nrow(pairs))) {
    ...
    # fig_slice <- ggplot(plot_df, aes(variable, value, group = slice, col = !!sym(arrangement_colname))) +
    #   geom_line() +
    #   labs(x = "radius", y = metric) +
    #   theme_bw()
...
plot_gradient_metrics_type2_boxplot <- function(spes_table, gradient_metric_df, slices_gradient_metric_df, metric, arrangement_colname, min_radius, max_radius) {
  ...
  for (i in seq(nrow(pairs))) {
    ...
    all_plots_list[[pairs[i, "pair"]]] <- list(fig_boxplot = fig_boxplot + theme(legend.position = "none"))
  }
  # Combine the plots together by reference cell type
  plots_pair_list <- list()
...
plot_proportion_SAC <- function(spes_table, SAC_df, slices_SAC_df, arrangement_colname) {
  ...
  for (i in seq_len(nrow(prop_cell_types))) {
    ...
    slices_SAC_df_subset$SAC <- joint_df$proportion_2D - joint_df$proportion_3D
...
plot_entropy_SAC <- function(spes_table, SAC_df, slices_SAC_df, arrangement_colname) {
  ...
  for (i in seq_len(nrow(entropy_cell_types))) {
    ...
    slices_SAC_df_subset$SAC <- joint_df$entropy_2D - joint_df$entropy_3D
...
plot_proportion_prevalence <- function(spes_table, prevalence_df, slices_prevalence_df, arrangement_colname) {
  ...
  for (i in seq_len(nrow(prop_cell_types))) {
    ...
    for (threshold_colname in threshold_colnames) {
      slices_prevalence_df_subset[ , threshold_colname] <- joint_df[ , paste(threshold_colname, "_2D", sep = "")] - joint_df[ , paste(threshold_colname, "_3D", sep = "")]
    }
...
plot_entropy_prevalence <- function(spes_table, prevalence_df, slices_prevalence_df, arrangement_colname) {
  ...
  for (i in seq_len(nrow(entropy_cell_types))) {
    ...
    plots <- plot_grid(all_plots_list[[cell_types]]$arrangement,
                       all_plots_list[[cell_types]]$slice,
                       # all_plots_list[[cell_types]]$bg_prop_A,
                       # all_plots_list[[cell_types]]$bg_prop_B,
                       # all_plots_list[[cell_types]]$shape, 
                       # all_plots_list[[cell_types]]$variation_E,
...
plot_proportion_prevalence_AUC <- function(spes_table, prevalence_df, slices_prevalence_df, arrangement_colname) {
  ...
  for (i in seq_len(nrow(prop_cell_types))) {
    ...
    slices_prevalence_df_subset$AUC <- joint_df$AUC_2D - joint_df$AUC_3D
...
# Boxplots
mixed_MS_plot <- plot_gradient_metrics_type1_boxplot(mixed_spes_table, mixed_MS_df, mixed_slices_MS_df, "MS", "cluster_prop_B")
mixed_NMS_plot <- plot_gradient_metrics_type1_boxplot(mixed_spes_table, mixed_NMS_df, mixed_slices_NMS_df, "NMS", "cluster_prop_B")
mixed_ACINP_plot <- plot_gradient_metrics_type1_boxplot(mixed_spes_table, mixed_ACINP_df, mixed_slices_ACINP_df, "ACINP", "cluster_prop_B")
mixed_AE_plot <- plot_gradient_metrics_type1_boxplot(mixed_spes_table, mixed_AE_df, mixed_slices_AE_df, "AE", "cluster_prop_B")
...
# Lines
mixed_ACIN_plot <- plot_gradient_metrics_type2(mixed_spes_table, mixed_ACIN_df, mixed_slices_ACIN_df, "ACIN", "cluster_prop_B", 10, 100)
mixed_CKR_plot <- plot_gradient_metrics_type2(mixed_spes_table, mixed_CKR_df, mixed_slices_CKR_df, "CKR", "cluster_prop_B", 30, 100)

# Box plots
mixed_ACIN_plot <- plot_gradient_metrics_type2_boxplot(mixed_spes_table, mixed_ACIN_df, mixed_slices_ACIN_df, "ACIN", "cluster_prop_B", 10, 100)
mixed_CKR_plot <- plot_gradient_metrics_type2_boxplot(mixed_spes_table, mixed_CKR_df, mixed_slices_CKR_df, "CKR", "cluster_prop_B", 30, 100)
...
# Boxplots
mixed_prop_prevalence_plot <- plot_proportion_prevalence_boxplot(mixed_spes_table, mixed_prop_prevalence_df, mixed_slices_prop_prevalence_df, "cluster_prop_B")
mixed_entropy_prevalence_plot <- plot_entropy_prevalence_boxplot(mixed_spes_table, mixed_entropy_prevalence_df, mixed_slices_entropy_prevalence_df, "cluster_prop_B")
